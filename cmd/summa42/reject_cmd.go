...
func newRejectCommand(api control.API, jsonOutput *bool) *cobra.Command {
	var ownerKey string
	command := &cobra.Command{
		Use:   "reject <approval-id>",
		Short: "Reject a pending action with an Owner signature",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			approval, err := api.Reject(cmd.Context(), domain.ID(args[0]), signer)
			if err != nil {
				return err
			}
			if *jsonOutput {
	...
	command.Flags().StringVar(&ownerKey, "owner-key", "", "path to an existing Owner Ed25519 private key (default: SUMMA42_HOME/keys/owner.key)")
	return command
}
