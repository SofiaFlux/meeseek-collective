...
func TestUsefulWorkVerticalSlice(t *testing.T) {
	...
	root := t.TempDir()
	statePath := filepath.Join(root, "state", "meeseek.db")
...
func putTextEvidence(t *testing.T, ctx context.Context, box *meeseekruntime.Box, value, kind string) evidence.EvidenceObject {
	t.Helper()
...
func completeAndAccept(t *testing.T, ctx context.Context, box *meeseekruntime.Box, taskID, attemptID, verifierID, evidenceID domain.ID) {
	t.Helper()
