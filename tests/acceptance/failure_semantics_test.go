...
func newFixture(t *testing.T, hardLimit int64) *fixture {
	...
	box, err := meeseekruntime.Open(ctx, config)
	if err != nil {
		t.Fatal(err)
	}
...
func testCrashBeforeCompletionRecordDoesNotFakeCompletion(t *testing.T) {
	...
	restarted, err := meeseekruntime.Open(f.ctx, f.config)
	if err != nil {
		t.Fatal(err)
	}
