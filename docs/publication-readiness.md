...
- [x] Local-artifact ignore coverage was corrected for `.summa42/`; re-run
  `git check-ignore -v .summa42/config.json .summa42/state.db
  .summa42/evidence/example` before publication.
...
- [x] `go build ./cmd/summa42 ./cmd/summa42-box` completed with exit 0 in the
  Linux sandbox.
...
- [x] After the public transition, the authorized maintainer enabled GitHub
  Private Vulnerability Reporting (PVR) on 2026-09-22.
- [ ] The authorized maintainer must still configure notification settings and
  submit a harmless external PVR test report before treating PVR as the
