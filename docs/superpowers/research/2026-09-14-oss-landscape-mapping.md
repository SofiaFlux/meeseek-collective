# Summa42 — OSS Landscape Mapping

**Status:** Research complete; ready for implementation architecture decisions  
**Date:** 2026-09-14  
**Input:** `docs/superpowers/specs/2026-09-14-summa42-design.md`
...
This document maps the approved Summa42 design onto the current open-source ecosystem.
...
- **ADOPT** — use an existing project substantially as-is behind a Summa42 contract.
- **ADAPT** — reuse an existing subsystem, engine, protocol, or implementation behind a Summa42-owned semantic adapter.
- **BUILD** — implement Summa42-owned semantics because they are differentiating, cross-cutting, security-critical, or not safely supplied by an existing project.
...
1. **Semantic fit** — does the project implement the meaning Summa42 requires, not merely a similarly named feature?
...
No single numeric score is used. A project can be excellent software and still be the wrong owner of a Summa42 semantic boundary.

---

## 3. Executive Decision

There is **no existing OSS project that should be forked and renamed into Summa42**.

The recommended shape is:

```text
Summa42-owned semantic core
│
...
└── BUILD the organizational semantics that make Summa42 unique
```
...
> **Summa42 should own semantics and conformance; OSS projects should supply engines and implementations behind those semantics.**

---

## 4. Mapping Summary

| Summa42 contract / concern | Decision | Primary candidate(s) | Rationale |
|---|---|---|---|
| `DurableWork` / runtime reliability | **ADAPT + BUILD** | AgentLedger; DBOS; Hatchet; Temporal | AgentLedger is unusually close to Summa42 failure semantics, but Summa42 still owns Task/Attempt/External Operation, purpose, acceptance and scheduler semantics. |
| Canonical Task/Attempt/External Operation model | **BUILD** | — | Differentiating core semantics; must match approved spec exactly. |
| `OperationReconciler` | **BUILD + ADAPT** | AgentLedger patterns/ledger | Logical-effect identity, confirmed/unknown effect reuse and reconciliation are core Summa42 guarantees. |
| `CollectiveMemory` | **BUILD + ADAPT** | Graphiti primary; Caura/Cognee alternatives | Summa42 owns evidence/epistemic semantics; graph system can provide temporal projection and retrieval. |
| `EvidenceStore` | **BUILD contract; ADOPT substrate** | Local files/object storage; AgentLedger evidence patterns | Evidence meaning/provenance is Summa42-owned; blob persistence is commodity infrastructure. |
| `PolicyEngine` | **ADAPT** | OPA primary; Cedar secondary | Reuse deterministic policy evaluation, build Summa42 decision schema and constitutional hierarchy around it. |
| Relationship authorization | **DEFER / optional ADAPT** | OpenFGA | Useful later for relationship-heavy resource access, but not sufficient as Summa42 policy engine. |
...
| `Scheduler` | **BUILD** | Hatchet/Temporal as optional execution backends | Summa42 arbitration semantics are unique and should not be delegated to a generic workflow scheduler. |
...
| `CapabilityProvider` | **BUILD + ADAPT** | MCP | MCP is excellent interoperability plumbing but capability != authority; assessment and enforcement remain Summa42-owned. |
| `InteractionProvider` | **BUILD adapters** | MCP + service-specific APIs | Interaction semantics, human attention and commit boundaries are Summa42-specific. |
...
| `CostProvider` | **BUILD semantics + ADOPT telemetry** | OpenTelemetry + provider billing APIs | OTel transports observations; Resource Ledger, reservations and spend exposure remain Summa42 semantics. |
...
AgentLedger is the most significant finding in this mapping because its scope overlaps heavily with the **hard reliability substrate** of the approved Summa42 design.
...
That alignment is unusually strong with the findings from the Summa42 written-design review.

### Why not ADOPT wholesale?

AgentLedger deliberately does **not** own several concepts that are fundamental to Summa42:

- Mission and authorized purpose;
- obligations and missionless work;
- Goal generation/Goal Market;
- Summa42-specific Task vs Attempt vs External Operation acceptance semantics;
...
Its model is a runtime reliability substrate for agent harnesses, while Summa42 is an autonomous organization runtime.

### Recommendation

Do not fork AgentLedger immediately.

Perform a **semantic compatibility spike** against the eight MVC failure-path scenarios. Determine whether Summa42 can:

1. run AgentLedger as an internal library/runtime adapter;
2. reuse its runtime contract/conformance ideas while implementing a Summa42-native state model;
...
AgentLedger is much younger than infrastructure such as Temporal/Postgres/OPA. The architecture should benefit from it without making Summa42 correctness dependent on one young project surviving indefinitely.
...
Limitations relative to Summa42:

- workflow-as-code semantics are not Summa42 Task/Attempt/External Operation semantics;
- no Summa42 authority hierarchy;
...
It does **not** replace the Summa42 scheduler because the approved scheduler includes semantics such as:
...
- its workflow model is not Summa42's organizational model;
- it does not provide Summa42 policy, memory, TEB or economic semantics;
...
No evaluated memory project should own canonical Summa42 truth.

Summa42 must retain ownership of:
...
Graphiti is highly aligned with Summa42's temporal knowledge requirements:
...
This directly maps to several approved Summa42 requirements.

### Boundary

Graphiti should not autonomously decide which Summa42 claims are `VERIFIED`, nor should its internal graph become the only retained evidence history.

Recommended pattern:

```text
Evidence / Events / Knowledge Delta (Summa42 canonical)
                │
...
6. custom Summa42 epistemic metadata;
...
It may be valuable if Summa42 later needs an off-the-shelf **shared semantic/social memory service**.

Reasons not to make it canonical today:

- its governance model is its own, not Summa42 Constitution/Authority semantics;
- its write path includes opinionated enrichment behavior;
- Summa42 requires evidence and epistemic lineage independent of any memory service;
...
Its default conceptual focus is more “AI memory/context” than Summa42's “evidence-driven organizational epistemology”.
...
- Summa42 memory must remain storage-product-neutral.
...
```text
Summa42 Authority Model + Policy Decision Contract
                     │
...
Compared with OPA, Cedar is more authorization-focused while Summa42 frequently needs richer policy outputs such as `REQUIRE_APPROVAL`, resource limits, risk constraints and semantic-effect decisions.
...
It is not by itself the right engine for Summa42's context-rich policy decisions, budget limits and safety semantics.
...
The Owner/root-human identity and constitutional ceremony are Summa42-specific security semantics.
...
Summa42's `Executor` contract is a core abstraction because it must normalize:
...
MCP is useful for Summa42 because many existing harnesses already understand it.

But:

> **MCP capability advertisement is not Summa42 authority.**
...
Generic workflow systems provide queues, timers, retries, affinity and priorities, but the approved Summa42 scheduler combines:
...
> **Summa42 owns scheduling decisions; external engines may execute the resulting decisions.**
...
This is an unusually good match for Summa42's “small system that can later distribute” principle.

It still must not own authoritative Summa42 semantics such as External Operation logical identity or Task acceptance.
...
Summa42 should emit OTel-compatible telemetry rather than inventing an observability protocol.
...
Those are semantic state and must remain durable in Summa42.

Recommendation:

```text
Summa42 Resource Ledger = source of economic semantics
...
LangGraph, CrewAI, AutoGen, Letta and similar projects can be useful **executors or internal planning libraries**, but should not own Summa42's runtime semantics.
...
- their memory models are not the full Summa42 epistemic model;
- Summa42 must remain vendor/harness neutral.
...
Goal: determine exactly what can be reused without weakening Summa42 semantics.
...
## Spike C — OPA Summa42 policy decision
...
Confirm that the Summa42 decision contract remains stable even if the underlying policy engine changes.
...
Verify that an MCP-discovered tool can be wrapped as a Summa42 capability while preserving:
...
1. **BUILD the Summa42 semantic core.** No current framework owns the approved organizational model.
...
3. **Keep canonical memory Summa42-owned; treat graph memory as a projection.** Graphiti is the first candidate to test.
...
1. What language/runtime owns the Summa42 semantic core?
