# Summa42 — MVC Implementation Architecture Decisions
...
The MVC must implement the approved Summa42 semantics without prematurely building the distributed future.
...
| Scheduler | Summa42-owned deterministic scheduler |
| Policy evaluation | embedded **OPA** behind Summa42 `PolicyEngine`, restricted side-effect-free profile |
| Enforcement | Summa42-owned **Trusted Enforcement Boundary** |
...
The Summa42 semantic core, Box daemon, CLI, scheduler, persistence layer, policy adapter, Resource Ledger, built-in capability providers, and built-in executor adapters are implemented in **Go**.
...
The exact Go SQLite driver is selected in the implementation plan after a compile/platform check, but the domain layer talks through a Summa42-owned persistence interface and transaction helpers rather than driver-specific APIs.
...
The transition from `PREPARED` to `DISPATCHED` is the point at which Summa42 accepts that an external effect may escape local control.
...
- a comparison target for Summa42 failure tests;
...
Where useful, Summa42 tests should be inspired by/compatible with generic AgentLedger invariants such as fencing and unknown side effects, while adding the Summa42-specific requirements:
...
Stable Summa42 `PolicyDecision` contains conceptually:
...
Policies are Summa42-managed versioned artifacts/state.
...
- external/current facts needed by policy are gathered by trusted Summa42 code and passed through controlled `input`/`data`;
...
MVC stores canonical memory in SQLite using Summa42-owned records for:
...
The Resource Ledger is canonical Summa42 state in SQLite.
...
May become physical execution backends if they provide value, but do not own the logical Summa42 Scheduler/Task semantics.
...
```text
cmd/
  summa42/              CLI entrypoint
  summa42-box/          optional dedicated daemon entrypoint if split binary is useful

internal/
  domain/               pure Summa42 domain types/invariants
...
- stable Summa42 `PolicyDecision`/input DTOs;
