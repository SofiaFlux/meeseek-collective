...
- `internal/control/`, `cmd/summa42/` — operator inspection / emit / approve / reject / experience surface.
- `internal/localconfig/` — non-secret field-feedback settings and maintenance envelope ID.
- `cmd/summa42-box/` — provider/config wiring.
...
- Modify: `cmd/summa42/approve_cmd.go`
- Create: `cmd/summa42/reject_cmd.go`
...
```bash
go test ./internal/approvals ./internal/control ./cmd/summa42 -count=1
```
...
`summa42 approve <id>` and `summa42 reject <id>` load the existing Owner key, sign V2 material, and call exact endpoints.
...
```bash
git add internal/approvals internal/control cmd/summa42
...
- stable marker is `summa42-feedback:<fingerprint>`;
...
```bash
go test ./internal/feedbackgithub ./cmd/summa42-box -count=1
```

- [ ] **Step 7: Commit**

```bash
git add internal/feedbackgithub cmd/summa42-box
...
- Modify: `internal/control/server_test.go`
- Create: `cmd/summa42/feedback_cmd.go`
- Create: `cmd/summa42/feedback_cmd_test.go`
- Create: `cmd/summa42/approvals_cmd.go`
- Modify: `cmd/summa42/root.go`
...
CLI:
- `summa42 feedback list`
- `summa42 feedback inspect <id>`
- `summa42 feedback emit <id>`
- `summa42 feedback observe --category <category> --summary <local-sensitive-summary> [--task <id>]`
- `summa42 feedback scan`
- `summa42 approvals list`
- existing `approve`
- new `reject`
...
```bash
go test ./internal/control ./cmd/summa42 -run 'Feedback|Approval|Reject' -count=1
```
...
```bash
git add internal/control cmd/summa42
...
- Create: `cmd/summa42/experience_cmd.go`
- Modify: `internal/control/server.go`
- Modify: `internal/control/client.go`
- Modify: `internal/control/server_test.go`
...
- `GET /experience`
- `GET /experience/{id}`
- `summa42 experience grant request ...`
- Owner uses the existing generic `summa42 approve <approval-id>`
- `summa42 experience grant activate <grant-request-id>`
...
```bash
git add internal/experience internal/control cmd/summa42
...
- Modify: `internal/runtime/box.go`
- Modify: `cmd/summa42-box/main.go`
- Modify: `cmd/summa42-box/main_test.go`
...
Wire `FieldObserver.Scan` into the control service used by `summa42 feedback scan`. The scanner reads canonical state/events and remains idempotent; it does not subscribe to an in-memory event bus and does not make primary Task acceptance depend on feedback logic.
...
```bash
go test ./internal/runtime ./cmd/summa42-box ./internal/control -count=1
```

- [ ] **Step 8: Commit**

```bash
git add internal/runtime cmd/summa42-box internal/observability README.md
