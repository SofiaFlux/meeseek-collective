# Summa42 MVC Implementation Plan — Self-Review Addendum
...
`internal/runtime/box.go` owns Box composition. `cmd/summa42-box/main.go` is only process/bootstrap wiring and must not become the service locator.
...
If a later policy genuinely needs another pure built-in, adding it is a reviewed policy-capability-profile change: add a unit test proving it is side-effect-free for Summa42's threat model, then change the profile hash/version. Do not silently widen capabilities at runtime.
...
`cmd/summa42-box/main.go` loads configuration, creates `runtime.NewBox`, handles OS signals, and calls `Run`. It contains no semantic SQL or policy logic.
