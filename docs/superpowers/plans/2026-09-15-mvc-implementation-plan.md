# Meeseek Collective MVC Implementation Plan
...
**Spec:** `docs/superpowers/specs/2026-09-14-meeseek-collective-design.md`
...
cmd/meeseek/main.go
cmd/meeseek/root.go
cmd/meeseek/init_cmd.go
cmd/meeseek/status_cmd.go
cmd/meeseek/task_cmd.go
cmd/meeseek/approve_cmd.go
cmd/meeseek/inspect_cmd.go
cmd/meeseek-box/main.go
...
- Create: `cmd/meeseek/main.go`
- Create: `cmd/meeseek/root.go`
- Create: `cmd/meeseek-box/main.go`
- Create: `internal/version/version.go`
- Test: `internal/version/version_test.go`

**Interfaces:**
- Produces: two buildable entry points, `meeseek` and `meeseek-box`.
...
```go
module github.com/SofiaFlux/meeseek-collective
...
Use Cobra for `meeseek`, but keep command wiring thin:
...
`cmd/meeseek/main.go` calls `Execute()` and exits non-zero on error. `cmd/meeseek-box/main.go` initially starts and exits cleanly with a placeholder-free `run(ctx)` function that later tasks extend.
...
steps:
  ...
  - run: go test ./...
  - run: go vet ./...
  - run: go build ./cmd/meeseek ./cmd/meeseek-box
```
...
git commit -m "build: bootstrap Meeseek Go module"
```
...
git commit -m "feat: define core Meeseek domain states"
```
...
- Create: `cmd/meeseek/init_cmd.go`
...
go run ./cmd/meeseek init --home "$(mktemp -d)"
```
...
```bash
git add internal/identity internal/bootstrap internal/state/sqlite/migrations cmd/meeseek
...
If `codex` is installed and `MEESEEK_CODEX_SMOKE=1`, run an ephemeral low-risk prompt in a temporary workspace and require clean JSONL completion. CI does not require external credentials for ordinary test runs.
...
A Meeseek submits observations/claims/evidence links; service validates schema/provenance, deduplicates entities/claims conservatively, records contradiction/supersession, and never upgrades a claim to VERIFIED solely because several agents repeat it.
...
- Create: `cmd/meeseek/status_cmd.go`
- Create: `cmd/meeseek/task_cmd.go`
- Create: `cmd/meeseek/approve_cmd.go`
- Create: `cmd/meeseek/inspect_cmd.go`
- Modify: `cmd/meeseek/root.go`
- Modify: `cmd/meeseek-box/main.go`
- Test: `internal/control/server_test.go`
- Test: `cmd/meeseek/root_test.go`
...
Run: `go test ./internal/control ./cmd/meeseek`
...
`meeseek status`, `task create`, `task show`, `approve`, `inspect attempt`, `inspect operation`. Output defaults human-readable; `--json` returns stable JSON DTOs.
...
Create a constructor in `cmd/meeseek-box` or a focused runtime package if necessary, wiring:
...
```bash
git add tests README.md cmd/meeseek-box .github/workflows/ci.yml
...
- Box can remain running with zero active Meeseeks and negligible active cognition;
