...
5. Verdict mapping: UNCERTAIN → hold `uncertain-review`; CLEAN+len(findings)>0 → hold `verdict-findings-mismatch`; CLEAN → propose approve; FINDINGS → propose comment. After the existing grant-action check, require the branch effect capability (`ado.pr.approve` or `ado.pr.comment`) ∈ grant capabilities, else hold `grant-denies-capability:<cap>`.
...
7. Next: Kind "publish-decision", caps/ceiling copied from input.Case.NextWork, then append the checked effect capability to both, ProposedActions per branch. Signature `ado:<ObjectID>:<RevisionID>:<verdict>` where ObjectID = repo#pr, RevisionID = source:target (rebuild from input, not methods — same encoding).
