...
- [ ] **Step 1: Write a failing iterative test.** `TestReviewThenPublishThenVerify` sends a `Continue` assessment after `copilot-review` with next kind `publish-decision`, then a `Ready` assessment after publication. Assert the first decision is `CONTINUE`, the second is `READY_FOR_VERIFICATION`, and that `Decide` itself never dispatches an effect. Use `Grant{Capabilities: []string{"ado.read", "ado.write"}, Actions: []string{"ado.pr.comment", "ado.pr.approve"}}`; give the second work only `ado.write` capability and one proposed action.
- [ ] **Step 2: Write a second failing recipe test.** `TestDocumentReviewThenArchiveThenVerify` uses different kinds (`read-document`, `archive-report`) and different capability/action names; run the same `Continue` then `Ready` sequence. Assert no ADO-specific string occurs in `decision.go`.
...
- [ ] **Step 4: Inspect the package boundary.** Confirm `decision.go` imports no ADO or Copilot package and performs no I/O; the different recipes must use the same `Decide` function.
