...
**Architecture basis:** `docs/superpowers/specs/2026-09-14-summa42-design.md`
...
- a Maintainer Collective that autonomously changes or releases Summa42;
...
`summa42 feedback emit <feedback-id>   # schedules/wakes governed COLLECTIVE_MAINTENANCE work` requests or wakes this governed Task; it is never a direct provider call.
...
- include a stable non-sensitive dedupe marker such as `summa42-feedback:<fingerprint>`;
...
This general mechanism replaces the production `unavailableApprovalService` adapter and makes the already-existing `summa42 approve` path durable.
...
```text
summa42 task create ... --task-class <local-scope>

summa42 feedback observe ...
summa42 feedback scan
summa42 feedback candidate ...
summa42 feedback sanitize <candidate-id>
summa42 feedback list
summa42 feedback inspect <candidate-id>
summa42 feedback emit <candidate-id>

summa42 approvals
summa42 approve <approval-id>
summa42 reject <approval-id>

summa42 experience grant request ...
summa42 experience grant activate <grant-request-id>
summa42 experience propose ...
summa42 experience observe-outcome <task-id>
summa42 experience evaluate <proposal-id>
summa42 experience list
summa42 experience inspect <rule-id>
```
...
Its mission may be to maintain Summa42 through:
