# Summa42 — Design Specification

**Status:** Approved written design; ready for OSS landscape mapping  
**Date:** 2026-09-14  
**Project:** Summa42
**Repository:** `SofiaFlux/summa42`
**Intended open-source license:** Apache License 2.0  
**Tagline:** **Kill the Summa42. Keep the mission.**

---

## 1. Executive Summary

Summa42 is a vendor-neutral runtime for turning heterogeneous executors—LLM harnesses, deterministic scripts, APIs, infrastructure tools, solvers, and other software—into a durable, autonomous, policy-bounded organization of ephemeral workers.

The core abstraction is not an AI agent. It is a **Collective** with durable identity, memory, authority, goals, obligations, work, audit, and economic awareness. A **Summa42** is an ephemeral logical worker responsible for an attempt at a task. The worker may be an LLM, a script, a deterministic function, SQL, Terraform, an API call, or another mechanism. Tasks and knowledge survive workers.
...
An ephemeral logical unit of work responsible for one task attempt. A Summa42 is **not synonymous with an LLM**.
...
- an **Attempt** is one execution effort against a Task, performed by a Summa42 under a lease generation;
...
5. **Summa42s inherit knowledge, not context windows.**
...
A Summa42 does not need to directly spawn a local child process. It creates durable child tasks in the Collective. The scheduler may run them on any eligible Cube.
...
Summa42s propose a **Knowledge Delta** rather than directly rewriting truth. A delta may contain:
...
Resource attribution should reach at least the level of a Summa42 Attempt where useful, while allowing aggregation when measurement overhead would exceed the value of precision.
...
An **Emissary** is a role/capability/Summa42 that communicates across Collective boundaries. It need not be a dedicated Cube.
...
Additional Summa42s are added while marginal value exceeds marginal cost and coordination overhead.
...
A Summa42 may conclude that the task, parent, goal, or Mission assumption is incorrect.
...
A Summa42 with a valid capability lease may directly use an allowed implementation such as:
...
Each Task/Summa42 receives the minimum capability set needed for its work, expiring with the Task or lease where technically possible.
...
The Box or Capability Provider should hold credentials outside the Summa42 when possible and perform the operation on its behalf.
...
A fully valid new Collective may contain zero active Summa42s and immediately enter DORMANT.
...
3. **Durable Task Graph** — work survives Summa42s and Box restart; Tasks and Attempts are separate, with lineage, child Tasks, deadlines, failure classes, challenge, and acceptance.
...
5. **Executor abstraction** — at least two meaningfully different executor types, preferably one agentic harness and one deterministic executor, proving `Summa42 != LLM`.
...
14. **Dormancy/wake semantics** — no useful work can correctly mean zero active Summa42s.
...
- **Kill the Summa42. Keep the mission.**
- **Tasks are durable; workers are disposable.**
- **Summa42 is a unit of work, not a synonym for LLM.**
...
- **Summa42s inherit knowledge, not context windows.**
...
5. A `Summa42` is not a long-lived identity whose survival should be optimized.
...
- **ADOPT** — an existing project sufficiently implements the required semantics and can be used behind a Summa42 contract;
- **ADAPT** — an existing project supplies a strong subsystem that needs an adapter, extension, or constrained semantic layer;
- **BUILD** — the required behavior is core to Summa42 or existing projects do not satisfy the contract.
